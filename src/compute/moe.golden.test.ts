/**
 * Golden gate: the TS MoE forward must match training/llmviz_train/moe.py
 * on every recorded activation (router probs, top-k gates, all expert
 * outputs, combined), for both golden inputs, within 1e-4 — real weights.
 */
...
import { moeDimsFrom, moeForward } from "./moe";
...
describe("moeForward vs PyTorch goldens (real weights)", () => {
  ...
  for (let g = 0; g < goldens.inputs.length; g++) {
    describe(`golden input ${g}`, () => {
      ...
      const logits = moeForward(weights, dims, input.tokens, rec);
