/**
 * Golden gate: the TS KAN forward must match training/llmviz_train/kan.py
 * on every recorded activation (rbf basis, spline, base, out), for both
 * golden inputs, within 1e-4 — using the real committed weights.
 */
...
import { kanDimsFrom, kanForward } from "./kan";
...
describe("kanForward vs PyTorch goldens (real weights)", () => {
  ...
  for (let g = 0; g < goldens.inputs.length; g++) {
    describe(`golden input ${g}`, () => {
      ...
      const logits = kanForward(weights, dims, input.tokens, rec);
