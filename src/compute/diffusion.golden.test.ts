/**
 * Golden gate: the TS diffusion denoiser must match
 * training/llmviz_train/diffusion.py on every recorded activation, for both
 * golden inputs, within 1e-4. Each golden input is a sequence with the four
 * answer positions masked; the denoiser must recover the data tokens there.
 */
...
import { diffusionDimsFrom, diffusionForward } from "./diffusion";
...
describe("diffusionForward vs PyTorch goldens (real weights)", () => {
  ...
  for (let g = 0; g < goldens.inputs.length; g++) {
    describe(`golden input ${g}`, () => {
      ...
      const logits = diffusionForward(weights, dims, input.tokens, rec);
      ...
      it("recovers the masked answer tokens (denoiser: predicted at the masked positions)", () => {
        expect(logits.shape).toEqual([input.tokens.length, dims.vocab_size]);
        const preds = argmax(logits);
        // Denoiser predicts in place: answers sit at the masked tail positions.
        const start = input.tokens.length - input.answer.length;
