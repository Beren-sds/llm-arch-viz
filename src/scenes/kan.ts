/**
 * KAN 3D scene — a GPT whose feed-forward is a FastKAN (learnable edge
 * functions as sums of Gaussian bumps). Same vertical-spine grammar;
 * attention clusters identical to the GPT scene. The KAN band shows the
 * RBF basis activations (the learnable edge functions, evaluated — the
 * hero), the spline and base branches, and their sum. Values are live from
 * compute/kan.ts (golden-gated).
 */
...
function formulaFor(name: string): string | undefined {
  ...
  if (s === "kan.rbf") return "exp(−((x̂−grid)/h)²) per edge";
  if (s === "kan.spline") return "spline = rbf·W_spline";
  if (s === "kan.base") return "base = silu(x̂)·W_base";
  if (s === "kan.out") return "x ← x + spline + base";
  return undefined;
}
...
function weightLabel(name: string): string {
  ...
  if (/^kans\.\d+\.spline_linear\.weight$/.test(name)) return "spline.W";
  if (/^kans\.\d+\.base_linear\.weight$/.test(name)) return "base.W";
  if (/^kans\.\d+\.base_linear\.bias$/.test(name)) return "base.b";
  const at = /^attns\.\d+\.(.+)$/.exec(name);
  if (at) return at[1].replace(".weight", ".W").replace(".bias", ".b");
  return name;
}

export function buildKanScene(deps: KanSceneDeps): SceneController {
  const { weights, manifest, picker } = deps;
  const dims = kanDimsFrom(manifest);
  const SEQ = KAN_SEQ_LEN;
  const dM = dims.d_model;
  const nHead = dims.n_head;
  const headDim = dims.head_dim;
  const dRbf = dM * dims.num_grids;
  ...
  addLabel(deps.i18n?.t("scene.kan.title") ?? "KAN — selective copying", 0, y + 30, {
    size: 12,
    color: "#d7e3ff",
  });
  ...
  for (let i = 0; i < dims.n_layer; i++) {
    ...
    // -- attention band (identical to the GPT/MoE scenes) ---------------------
    ...
    const kanOutRect = spine(L("kan.out"), dM);

    const kTop = ln2Rect.top;
    let kx = spineHalf + FLANK_MARGIN;
    const rbf = place(L("kan.rbf"), [SEQ, dRbf], { leftX: kx, topY: kTop }); // wide hero
    kx = rbf.right + LANE_GAP;
    const splineW = placeW(`kans.${i}.spline_linear.weight`, { leftX: kx, topY: kTop });
    kx = splineW.right + LANE_GAP;
    const spline = place(L("kan.spline"), [SEQ, dM], { leftX: kx, topY: kTop });
    kx = spline.right + LANE_GAP;
    const base = place(L("kan.base"), [SEQ, dM], { leftX: kx, topY: kTop });
    kx = base.right + LANE_GAP;
    const baseW = placeWB(`kans.${i}.base_linear.weight`, `kans.${i}.base_linear.bias`, { leftX: kx, topY: kTop });
    y = Math.min(kanOutRect.bottom, union(rbf, splineW, spline, base, baseW).bottom) - REGION_GAP - SPINE_GAP;
  }
  ...
  for (let i = 0; i < dims.n_layer; i++) {
    ...
    wire(L("ln2.out"), L("kan.rbf"), "Gaussian basis");
    wire(L("kan.rbf"), L("kan.spline"), "× Wₛ");
    wire(L("kan.spline"), L("kan.out"), "+ base");
  }
  ...
  const r = (name: string): Rect => {
    ...
    if (!rect) throw new Error(`kan scene: no rect for "${name}"`);
    return rect;
  };
  ...
  function applyActivations(acts: Map<string, T>): void {
    for (const [name, t] of acts) {
      ...
      if (!view) throw new Error(`kan scene: no view for activation "${name}"`);
      view.setValues(t);
    }
  ...
  function runTokens(toks: number[]): Map<string, T> {
    ...
    kanForward(weights, dims, toks, rec);
    return rec.activations;
  }
  const binding: SceneBinding = {
    runForward(uptoToken: number): Map<string, T> {
      if (tokens.length === 0) throw new Error("kan scene: runForward before setTokens");
      if (!Number.isInteger(uptoToken) || uptoToken < 0 || uptoToken >= tokens.length) {
        throw new Error(`kan scene: uptoToken ${uptoToken} out of range [0, ${tokens.length})`);
      }
    ...
    setHighlight(names: string[], on: boolean): void {
      for (const name of names) {
        const view = views.get(name);
        if (!view) throw new Error(`kan scene: setHighlight on unknown "${name}"`);
  ...
  function setTokens(toks: number[]): void {
    if (toks.length < 1 || toks.length > SEQ) {
      throw new Error(`kan scene: setTokens needs 1..${SEQ} tokens, got ${toks.length}`);
    }
    for (const t of toks) {
      if (!Number.isInteger(t) || t < 0 || t >= dims.vocab_size) {
        throw new Error(`kan scene: token ${t} outside vocab [0, ${dims.vocab_size})`);
      }
    }
    tokens = toks.slice();
...
export function buildKanChapters(scene: SceneController, i18n: I18n): ChapterRegistry {
  const at = (anchor: string): CameraKeyframe => {
    const kf = scene.anchors.get(anchor);
    if (!kf) throw new Error(`kan chapters: scene has no anchor "${anchor}"`);
    return kf;
  };
  const chapters: Chapter[] = [
    { id: "intro", camera: at("home"), highlights: [], narrationKey: "kan.ch.intro.body" },
    {
      id: "embed",
      camera: at("embed"),
    ...
    },
    {
      ...
      narrationKey: "kan.ch.readout.body",
    },
  ];
  return new ChapterRegistry("kan", chapters, i18n);
}
