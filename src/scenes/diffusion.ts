/**
 * Text-diffusion 3D scene — a bidirectional masked denoiser. Same vertical-
 * spine grammar as the GPT scene (attention + MLP), but the attention is
 * non-causal (no masked triangle in the scores) and the INPUT has its four
 * answer positions replaced by [MASK]: the scene shows the denoiser filling
 * those masks in one pass. Values live from compute/diffusion.ts
 * (golden-gated).
 */
...
export const DIFFUSION_SEQ_LEN = 21;
/** Answer tokens occupy the last n_data positions (the task's teacher-forced tail). */
const ANSWER_POSITIONS = 4;
...
export interface DiffusionSceneDeps {
  scene: THREE.Scene;
...
function formulaFor(name: string): string | undefined {
  if (name === "embed.out") return "x = TokEmb[t] + PosEmb[t]  (masked)";
  if (name === "final_norm.out") return "x̂ = LayerNorm(x)";
  if (name === "head.logits") return "logits = W_head·x̂  (predicts clean tokens)";
  ...
  if (s === "attn.scores") return "S = Q·Kᵀ/√d  (no mask — bidirectional)";
  if (s === "attn.weights") return "A = softmax(S)";
  if (s === "attn.out") return "x ← x + (A·V)·W_o";
  if (s === "mlp.fc") return "h = x̂·W_fc + b";
  if (s === "mlp.act") return "GELU(h)";
  if (s === "mlp.proj") return "x ← x + GELU(h)·W_proj";
  return undefined;
}
...
function weightLabel(name: string): string {
  ...
  const mp = /^mlps\.\d+\.(.+)$/.exec(name);
  if (mp) return mp[1].replace(".weight", ".W").replace(".bias", ".b");
  return name;
}

export function buildDiffusionScene(deps: DiffusionSceneDeps): SceneController {
  const { weights, manifest, picker } = deps;
  const dims = diffusionDimsFrom(manifest);
  const SEQ = DIFFUSION_SEQ_LEN;
  ...
  addLabel(deps.i18n?.t("scene.diffusion.title") ?? "Diffusion — selective copying", 0, y + 30, {
    size: 12,
    color: "#d7e3ff",
  });
  ...
  for (let i = 0; i < dims.n_layer; i++) {
    ...
    const fcW = placeWB(`mlps.${i}.fc.weight`, `mlps.${i}.fc.bias`, { leftX: mx, topY: mTop });
    mx = fcW.right + LANE_GAP;
    const fc = place(L("mlp.fc"), [SEQ, dMlp], { leftX: mx, topY: mTop });
    const act = place(L("mlp.act"), [SEQ, dMlp], { leftX: mx, topY: fc.bottom - FLANK_GAP });
    mx = union(fc, act).right + LANE_GAP;
    const projW = placeWB(`mlps.${i}.proj.weight`, `mlps.${i}.proj.bias`, { leftX: mx, topY: mTop });
    y = Math.min(projOutRect.bottom, union(fcW, act, projW).bottom) - REGION_GAP - SPINE_GAP;
  }
  ...
  const r = (name: string): Rect => {
    ...
    if (!rect) throw new Error(`diffusion scene: no rect for "${name}"`);
    return rect;
  };
  ...
  function applyActivations(acts: Map<string, T>): void {
    for (const [name, t] of acts) {
      ...
      if (!view) throw new Error(`diffusion scene: no view for activation "${name}"`);
      view.setValues(t);
    }
  ...
  const binding: SceneBinding = {
    ...
    setHighlight(names: string[], on: boolean): void {
      for (const name of names) {
        const view = views.get(name);
        if (!view) throw new Error(`diffusion scene: setHighlight on unknown "${name}"`);
  ...
  function setTokens(toks: number[]): void {
    if (toks.length < 1 || toks.length > SEQ) {
      throw new Error(`diffusion scene: setTokens needs 1..${SEQ} tokens, got ${toks.length}`);
    }
    for (const t of toks) {
      if (!Number.isInteger(t) || t < 0 || t >= dims.vocab_size) {
        throw new Error(`diffusion scene: token ${t} outside vocab [0, ${dims.vocab_size})`);
      }
    }
    tokens = toks.slice();
...
export function buildDiffusionChapters(scene: SceneController, i18n: I18n): ChapterRegistry {
  const at = (anchor: string): CameraKeyframe => {
    const kf = scene.anchors.get(anchor);
    if (!kf) throw new Error(`diffusion chapters: scene has no anchor "${anchor}"`);
    return kf;
  };
  const chapters: Chapter[] = [
    { id: "intro", camera: at("home"), highlights: [], narrationKey: "diffusion.ch.intro.body" },
    {
      id: "embed",
      camera: at("embed"),
